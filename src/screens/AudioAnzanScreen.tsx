...
// شاشة الأنزان السمعي
// ═══════════════════════════════════════════════════════════
// - 5 أسئلة من bank-v2 (سمعي)
// - TTS يقرأ الأرقام واحداً واحداً (3s/رقم)
// - بدون عرض بصري (يسمع فقط)
...
export function AudioAnzanScreen({
  ...
  // ═══════════════════════════════════════════════════════
  // TTS: قراءة الأرقام واحداً واحداً
  // ═══════════════════════════════════════════════════════
  useEffect(() => {
    if (phase !== 'listening') return;
    if (!currentQ) return;
    if (!isSupported) {
      // إذا لم يدعم الصوت — انتقل للإجابة مباشرة
      setPhase('answering');
      return;
    }
  ...
  const nextQuestion = useCallback(() => {
    ...
    playSound, sorobana, stopSpeech, onComplete, markAnzanAudioPassed,
  ]);
        ...
        >
          ...
          <div className="space-y-3 text-sm text-white/80 font-body">
            ...
            <div className="flex items-start gap-3">
              ...
              <p>الرقم الأول بلا إشارة، والباقي مع إشاراته (زائد، ناقص...)</p>
            </div>
            <div className="flex items-start gap-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <p>يمكنك إعادة السمع <strong>مرة واحدة</strong> لكل سؤال</p>
            </div>
            <div className="flex items-start gap-3">
          ...
          <div className="mt-5 p-4 rounded-2xl bg-amber-500/10 border border-amber-400/30">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-amber-200 font-body leading-relaxed">
                💡 <strong>تحذير:</strong> العدّاد يتوهج عند ٦٠٪ من الوقت.
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <div dir="rtl" className="px-3 sm:px-6 py-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between gap-3 mb-5">
          ...
          <button
            ...
            disabled={replayUsed}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-purple-500/20 border border-purple-400/40 text-purple-200 text-xs font-bold hover:bg-purple-500/30 disabled:opacity-40 disabled:cursor-not-allowed"
          >
